 ...
 *
 * Run:
 *   npm run mcp                   → stdio transport (for Claude Desktop / CLI)
 *   MCP_TRANSPORT=sse npm run mcp → HTTP SSE transport (for web agents)
 *
 * Tools:
...
server.tool(
  ...
  {
    agentEns: z.string().describe("Your ENS name (e.g. treasury-agent.sail.eth)"),
    inputHash: z
      .string()
    ...
    proposedAction: z
      .string()
      .describe("The exact action you will take (calldata, tx description, etc.)"),
    attestation: z
      .string()
      .optional()
...
server.tool(
  ...
  async ({ agentEns, commitmentHash }) => {
    const result = await pipeline.execute(agentEns, commitmentHash as `0x${string}`);
    return {
      content: [
        {
          type: "text",
...
server.tool(
  ...
  "Submit a transaction via the AXL encrypted P2P mesh. The transaction bytes are pre-committed on-chain before submission so any in-transit modification is detectable.",
  {
    txBytes: z
      .string()
      .describe("Hex-encoded transaction bytes to deliver"),
    recipientPeerId: z
      .string()
      .optional()
  ...
  async ({ txBytes, recipientPeerId }) => {
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ...
              axlBridgeUrl: process.env["AXL_BRIDGE_URL"] ?? "http://localhost:9002",
            }),
          },
        ],
        isError: true,
    ...
    return {
      content: [
        {
          type: "text",
          text: JSON.stringify({ delivered: true, via: "axl", recipient: recipientPeerId ?? "broadcast" }),
        },
      ],
    };
  },
...
server.tool(
  ...
  "Delegate a task to a worker agent via AXL encrypted P2P channel. Resolves the worker's ENS name to get its AXL peer ID, then sends the task payload. The worker runs the full SAIL pipeline and returns a commitmentHash proving it acted correctly.",
  {
    ...
    task: z
      .string()
      .describe("Task description / payload for the worker"),
    context: z
      .unknown()
      .optional()
...
server.tool(
  ...
  async ({ since }) => {
    ...
    if (!alive) {
      return {
        content: [{ type: "text", text: JSON.stringify({ messages: [], axlOnline: false }) }],
      };
    }
