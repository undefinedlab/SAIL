...
SAIL uses **0G Storage** as the decentralized audit trail for every AI agent commitment, and **0G Compute** as the sealed inference backbone for attestation tier agents. Together, they make AI agent decisions both immutable and provably honest.
...
Without 0G Storage, the encrypted decision blob would have to live somewhere centralized — a database, IPFS with a pinning service, or the blockchain itself. Databases can be deleted. IPFS pinning services go offline. Storing blobs onchain is prohibitively expensive.

Without 0G Compute, ZK-tier agents have no way to prove *how* they reasoned. The decision could be anchored onchain, but an agent could still run a different model, use different inputs, or fabricate the output entirely. The accountability is superficial.
...
- Any byte modification changes the root hash, invalidating the onchain anchor
...
Every time a SAIL agent commits a decision, the encrypted blob is uploaded to **0G Storage (Log mode)** on the Galileo testnet. The root hash returned by 0G becomes the `cid` anchored onchain in `SAIL.commit()`.
...
Later, any auditor:
  fetch blob from 0G using root_hash
    → decrypt
      → keccak256(plaintext) must equal commitmentHash onchain
```

**Tamper-evident by content-addressing** — any byte modification changes the Merkle root hash and invalidates the onchain anchor. The SAIL contract is the source of truth; 0G Storage is the evidence vault.
